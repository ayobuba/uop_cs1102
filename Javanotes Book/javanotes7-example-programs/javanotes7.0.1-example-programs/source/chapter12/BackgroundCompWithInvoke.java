 ...
 * 
 * This is a version of BackgroundComputationDemo that uses 
 * SwingUtilities.invokeAndWait() instead of synchronization to handle
 * coordinate between the computation thread and the GUI thread.
 */
public class BackgroundCompWithInvoke extends JPanel {
    ...
    public static void main(String[] args) {
        ...
        BackgroundCompWithInvoke content = new BackgroundCompWithInvoke();
     ...
     */
    public BackgroundCompWithInvoke() {
        ...
        public void run() {
            try {
                ...
                for (int row = 0; row < height; row++) {  // Compute one row of pixels.
                    ...
                    final int r = row; // A local variable used in an inner class must be
                                       // final, so we can't use row itself in the following.
                    
                    /* SwingUtilities.invokeLater schedules a Runnable to be executed in the
                     * GUI thread, which also executes the display's paintComponent method.
                     * So, all accesses to the image will be in the GUI thread, eliminating
                     * the need for synchronization.  This can improve performance.
                     */
                    
                    SwingUtilities.invokeLater( new Runnable() {
                        public void run() {
                            image.setRGB(0, r, width, 1, rgb, 0, width);
                            display.repaint(0,r,width,1); // Repaint just the newly computed row.
                        }
                    } );
                    
                    /* The following is an alternative to using SwingUtilities.invokeLater.
                     * SwingUtilities.invokeAndWait schedules a Runnable to be run in the
                     * GUI thread, then waits until the Runnable has finished executing.
                     * This might be necessary if it is impossible for this thread to proceed
                     * until the Runnable has run, or if Runnables could be generated faster
                     * than they can be handled by the GUI thread.
                     */
...
//                          // An error occurred in the run() method of the invoked Runnable.
//                          // This shouldn't be possible here.
//                        System.out.println("Unexpeted error in invoked Runnable: " + e.getCause());
//                    }
